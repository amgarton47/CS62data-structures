...
public class DLL_Node implements Iterable<DLL_Node> {
	protected DLL_Node next;	// forward pointer
	protected DLL_Node prev;	// backwards pointer
	
	// these enable testing of the most obvious race conditions
	public static boolean insert_race = false;
	public static boolean remove_race = false;
	...
	public void insert(DLL_Node after) {
		...
		if (insert_race)	// RACE CONDITION TESTING
			Thread.yield();
		after.next = this;
		if (insert_race)	// RACE CONDITION TESTING
			Thread.yield();
	...
	public void remove() {
		...
		if (this.next != this) {
			if (remove_race)	// RACE CONDITION TESTING
				Thread.yield();
	 ...
	 * 
	 * NOTE: the list is not locked during iteration, which means
	 * 		 that updates during iteration could result in an 
	 *		 incomplete or inconsistent iteration.
	 */
