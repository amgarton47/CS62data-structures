...
/**
 * this is a very basic test suite for the early stages of
 * a Darwin implementation.
 */
public class AutograderCompTest
{
  public AutograderCompTest() { System.out.println("Please note that this only tests compatibility, not correctness."); }

  /**
   * We create a world, place a string in a known place, and see
   * if reads back as we set it.
   */
  public void testWorld() {
    try {
      ...
      if (str.equals("yo"))
    	  ...
    	  System.out.println("World compatability test failed: get != set");
  ...
  /**
   * we test the Species class by reading in a know description
   * and see if it is correctly instantiated.
   */
  public void testSpecies() {
    try {
      BufferedReader in = new BufferedReader(new FileReader(
            "./Creatures/" + CREATURE_NAME + ".txt"));
      
      Species s = new Species(in);
      char c = s.getSpeciesChar();
      if (c != CREATURE_NAME.charAt(0)) {
    	  System.out.println("Species compatability test failed: incorrect species character");
    	  return;
      }
      String name = s.getName();
      if (!name.equals(CREATURE_NAME)) {
    	  System.out.println("Species compatability test failed: incorrect name readback");
    	  return;
      }
      String color = s.getColor();
      if (!color.equals(CREATURE_COLOR)) {
    	  System.out.println("Species compatability test failed: incorrect color readback");
    	  return;
      }
      int size = s.programSize();
      if (size != CREATURE_PGM_SIZE) {
    	  System.out.println("Species comparability test failed: incorrect program size");
    	  return;
      }
      Instruction step = s.programStep(1);
      if (step.opcode != CREATURE_FIRST_INST) {
    	  System.out.println("Species compatability test filed: incorrect first instruction");
    	  return;
      }
 ...
 */
  public void testCreature() {
    try {
        ...
        if (s1 != s) {
        	System.out.println("Creature compatability test failed: wrong species returned");
        	return;
        }
        ...
        if (pos1.getX() != pos.getX() || pos1.getY() != pos.getY()) {
        	System.out.println("Creature compatability test failed: wrong pos returned");
        	return;
        }
        ...
        int dir = c.direction();
        if (dir != CREATURE_DIR) {
        	System.out.println("Creature compatability test failed: wrong direction returned");
        	return;
        }
...
/**
 * to test the full program, we invoke the main module with
 * three files, and expect to see an appropriately populated
 * world.
 */
  ...
  /**
   * if this module is invoked as an application it runs a set of 
   * (very) basic unit tests.
   * 
   * As you add additional tests, it us up to you whether you want
   * to add them here, or create real JUnit test modules.
   */
