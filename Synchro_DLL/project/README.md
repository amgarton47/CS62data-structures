...
* Gain experience with Java *synchronized methods* and *classes*.

## Key Terms and Concepts

* `Race Condition` - a situation where the results of a computation depend on the
   scheduling or parallel activities.
...
can make changes to shared data, a few types of problems are likely to arise:
   ...
   1. A thread that is attempting to accumulate the state of a complex system
      may find that that its enumeration to yield an inconsistent state 
   ...
   2. If multiple updates are required to perform a single operation
      ...
      student course list and the class roster), another operation may
      find the system in an inconsistent state.

   3. If multiple multi-update operations are in progress at the same
...
which threads are executed is called a *race condition*:  
   ...
   * Other race conditions, however, can be more critical.  If two
     ...
     circuit back on to do some testing, the result could be fatal.
...
allowed multiple threads (or processes) top operate (in parallel) on
...
not *MT-safe* (cannot be safely used in a multi-threaded application).
...
In this lab you will:
   ...
   1. Examine the (quite simple) code in the `DLL_Node.insert()` and `DLL_Node.remove()`
      ...
      if multiple of these operations were inprogress at the same time.
   ...
   4. Explore a few approaches to synchronizing access to these critical
      sections.
...
is recognizing what data is at risk, more specifically:
...
Failures due to race conditions are merely a risk.  They are seldom
...
Any potential of such failure is unacceptable, but the likelihood of
a failure during any particular run is a matter of statistics.  The
...
them and stop.  If it finds no errors, it will stop and report 
...
command line.  If, for instance, we wanted to run ten threads with 32 threads, 
we might:
...
that, if bugs our present, our test cases will exercise them.

When you examined the code for `DLL_Node.insert()` and
`DLL_Node.remove()` you surely noticed the calls to `yield()`.
...
method can execute (on that object) at the same time.

Update the `DLL_Node` to make the `insert()` and `remove()` methods
to be `synchronized` and re-run your tests.  We expect that you will
find thAe results to be disappointing.  

Review the `DLL_Node` APIs and specifications for `synchronized` 
methods, and suggest an explanation for why this did not entirely
elminate the problem.

### 5. Java Block Synchronization

(without explaining why `synchronized` methods failed) It is 
(sadly) sometimes the case that this mechanism is inadequate
...
the operations within the `synchronized` block.  This can solve
the above-mentioned problems by:
   ...
   * providing atomic execution for multiple operations.
...
operations.  Add that synchronization, re-run you tests,
...
Most of the grading will be based on the prose in `analsys.txt`, 
