...
public class WordStream {
	...
	private int nextItem = 0; // index of next item to be returned from nextToken method
	
	// Characters that indicate the end of a word, including punctuation and "white space"
	...
	/**
	 * @param line string containing words to be placed in list of lexical items
	 * post: all lexical items (including punctuation have been added to list of items
	 */
	 ...
	 * @return true iff it corresponds to white space, i.e., space, newline, or tab.
	 */
	public boolean isWhiteSpace(char item) {
		return item == ' ' || item == '\n' || item == '\t';
	}
	...
	public String nextToken() {
		while (nextItem < lexItems.size() && 
					isWhiteSpace(lexItems.get(nextItem).charAt(0))) {
			nextItem++;
		}
	...
	/**
	 * @return true iff there are more tokens to be returned from list of lexical items
	 */
	public boolean hasMoreTokens() {
		return nextItem < lexItems.size();
	}
